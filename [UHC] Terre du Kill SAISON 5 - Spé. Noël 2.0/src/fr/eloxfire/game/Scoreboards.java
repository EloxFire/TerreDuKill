...
public class Scoreboards implements Listener {
	...
	public static void initObjectives() {
		...
		TDK.setDisplayName("§6[T.D.K] - SAISON 5");
	}
	...
	public static void initScores() {
		...
		String TEMPS = Timers.formatIntoHHMMSS(Timers.GENE);
		int PREADY = setGame.ready.size();
		...
		TJEU = TDK.getScore("§2Timer : " + TEMPS);
		TJEU.setScore(2);

		Spawn = TDK.getScore("§cSpawn : " + "X:" + setGame.LocX + " Z:" + setGame.LocZ);
		Spawn.setScore(1);
	...
	public static void refreshScoreboard() {
		...
		for (String ligne : s.getEntries()) {
			if (ligne.contains("§cSpawn : ")) {
				s.resetScores(ligne);
				String lastLigne = ligne.split(":")[0];
				String newLigne = lastLigne + ": " + "X:" + setGame.LocX + " Z:" + setGame.LocZ;
